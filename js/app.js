...
let userCloseButtons = document.querySelectorAll('.user-header span');

for (let index = 0; index < userIcons.length; index++) {
	const userIcon = userIcons[index];
	userIcon.addEventListener('click', function (e) {
		userIcons.forEach.call(userIcons, function (el) {
			el.parentElement.classList.remove('_active');
			e.preventDefault();
		});
		userIcon.parentElement.classList.toggle('_active');
		for (let index = 0; index < userCloseButtons.length; index++) {
			const userCloseButton = userCloseButtons[index];
			userCloseButton.addEventListener('click', function () {
				userIcon.parentElement.classList.remove('_active');
			});
		}
	});
}
...
for (const submenuLink of submenuLinks) {
	submenuLink.addEventListener('mouseover', function (e) {
		submenuLink.classList.add('_active');
	});
	submenuLink.addEventListener('mouseout', function (e) {
...
document.addEventListener(
	...
	function () {
		...
		document.querySelector('.header__progress').style.setProperty('--scroll', scrollPercent);
	},
	{ passive: true }
);
...
// Size
let sizes = document.querySelectorAll('.item__size');

for (let index = 0; index < sizes.length; index++) {
	const size = sizes[index];
	size.addEventListener('click', function (event) {
		const sizeId = event.currentTarget.getAttribute('data-size');
		const currentSizes = document.querySelectorAll(`[data-size="${sizeId}"]`);
		for (const currentSize of currentSizes) {
			currentSize.classList.remove('_active');
		}
...
// Favorite
let favorites = document.querySelectorAll('.item__favorite');
...
// Colors
let productColors = document.querySelectorAll('.item__color');
let productImages = document.querySelectorAll('.item__image');

for (let index = 0; index < productColors.length; index++) {
	const productColor = productColors[index];
	productColor.addEventListener('click', function (event) {
		const colorId = event.currentTarget.getAttribute('data-product');
		const colors = document.querySelectorAll(`[data-product="${colorId}"]`);
		for (const color of colors) {
			color.classList.remove('_active');
			const images = color.children;
			for (const image of images) {
				image.classList.remove('_active');
			}
		}
		productImages[index].classList.add('_active');
		productColor.classList.add('_active');
	});
}

// Random product data attrs
let products = document.querySelectorAll('.products__item');

for (let index = 0; index < products.length; index++) {
	const product = products[index];
	let rand = Math.random().toString(36).substr(2, 9);
	product.setAttribute('data-product', rand);
	const dataId = product.getAttribute('data-product');
	let colors = product.querySelectorAll('.item__color');
	for (let index = 0; index < colors.length; index++) {
		const color = colors[index];
		color.setAttribute('data-product', dataId);
	}
	let sizes = product.querySelectorAll('.item__size');
	for (let index = 0; index < sizes.length; index++) {
		const size = sizes[index];
		size.setAttribute('data-size', dataId);
	}
}
...
let offerSlider = new Swiper('.slider-offer__body', {
	autoplay: {
		delay: 4000,
	...
	loop: true,
	navigation: {
		nextEl: '.slider-offer__control-next',
		prevEl: '.slider-offer__control-prev',
	},
	on: {
...
let dropdownSlider = new Swiper('.slider-dropdown__body', {
	observer: true,
	observeParents: true,
	slidesPerView: 3,
	spaceBetween: 30,
	...
	loop: true,
	navigation: {
		nextEl: '.slider-dropdown__control-next',
		prevEl: '.slider-dropdown__control-prev',
	},
	on: {
		lazyImageReady: function () {
			ibg();
...
let tabs_m = document.querySelectorAll('._tabs_m');
for (let index = 0; index < tabs.length; index++) {
	let tab_m = tabs_m[index];
	let tabs_items_m = tab_m.querySelectorAll('._tabs-item_m');
	let tabs_blocks_m = document.querySelectorAll('._tabs-block_m');
	for (let index = 0; index < tabs_items_m.length; index++) {
		let tabs_item_m = tabs_items_m[index];
		tabs_item_m.addEventListener('click', function (e) {
			for (let index = 0; index < tabs_items_m.length; index++) {
				let tabs_item_m = tabs_items_m[index];
				tabs_item_m.classList.remove('_active');
				tabs_blocks_m[index].classList.remove('_active');
			}
			tabs_item_m.classList.add('_active');
			tabs_blocks_m[index].classList.add('_active');
			e.preventDefault();
		});
	}
}
